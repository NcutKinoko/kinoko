...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{route('search.product')}}" method="POST" class="card card-sm" style="margin-bottom: 16px;">
            {{ csrf_field() }}
            <div class="card-body row no-gutters align-items-center">
                <!--end of col-->
                <div class="col">
                    <input class="form-control form-control-lg form-control-borderless" name="search" type="search"
                           placeholder="搜尋產品名稱" required>
                </div>
                <!--end of col-->
                <div class="col-auto">
                ...
                @foreach($product as $products)
                    <tr>
                        ...
                        <td class="align-middle">
                            <a href="{{route('show.product.updateForm',$products->id)}}" class="btn btn-success"
                               style="display: inline-block">修改</a>
                            <form class="delete" action="{{route('destroy.product',$products->id)}}" method="POST"
